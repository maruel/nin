...
type MissingDependencyPrinter struct {
	//OnStats(nodes_processed, nodes_missing_deps, missing_dep_path_count, generated_nodes, generator_rules int)
}
...
func (m *MissingDependencyPrinter) OnMissingDep(node *Node, path string, generator *Rule) {
	//cout << "Missing dep: " << node.path() << " uses " << path << " (generated by " << generator.name() << ")\n"
}
...
func (m *MissingDependencyScanner) PrintStats() {
	//cout << "Processed " << len(m.seen_) << " nodes.\n"
	if m.HadMissingDeps() {
		//cout << "Error: There are " << m.missing_dep_path_count_<< " missing dependency paths.\n"
		//cout << m.nodes_missing_deps_.size() << " targets had depfile dependencies on " << m.generated_nodes_.size() << " distinct generated inputs " << "(from " << m.generator_rules_.size() << " rules) " << " without a non-depfile dep path to the generator.\n"

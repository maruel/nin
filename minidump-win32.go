...
typedef BOOL (WINAPI *MiniDumpWriteDumpFunc) ( IN HANDLE, IN DWORD, IN HANDLE, IN MINIDUMP_TYPE, IN CONST PMINIDUMP_EXCEPTION_INFORMATION, OPTIONAL IN CONST PMINIDUMP_USER_STREAM_INFORMATION, OPTIONAL IN CONST PMINIDUMP_CALLBACK_INFORMATION OPTIONAL )

// Creates a windows minidump in temp folder.
func CreateWin32MiniDump(pep *_EXCEPTION_POINTERS) {
  ...
  if mini_dump_write_dump == nil {
    Error("failed to create minidump: GetProcAddress('MiniDumpWriteDump'): %s", GetLastErrorString())
    return
  }
  ...
  if rv == nil {
    Error("MiniDumpWriteDump failed: %s", GetLastErrorString())
    return
  }

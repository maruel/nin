...
func (d *DyndepLoader) UpdateEdge(edge *Edge, dyndeps *Dyndeps, err *string) bool {
	...
	edge.Outputs = append(edge.Outputs, dyndeps.implicit_outputs_...)
	edge.ImplicitOuts += int32(len(dyndeps.implicit_outputs_))

	// Add this edge as incoming to each new output.
	for _, i := range dyndeps.implicit_outputs_ {
		if old_in_edge := i.InEdge; old_in_edge != nil {
			...
			if !old_in_edge.GeneratedByDepLoader {
				*err = "multiple rules generate " + i.Path
				return false
			}
			old_in_edge.Outputs = nil
		}
		i.InEdge = edge
	}
	...
	edge.Inputs = make([]*Node, len(edge.Inputs)+len(dyndeps.implicit_inputs_))
	copy(edge.Inputs, old[:offset])
	copy(edge.Inputs[offset:], dyndeps.implicit_inputs_)
	copy(edge.Inputs[offset+len(dyndeps.implicit_inputs_):], old[offset:])
	edge.ImplicitDeps += int32(len(dyndeps.implicit_inputs_))
